...
void Poly :: drawObj(HDC hDC, HPEN hPen, int R, int G, int B,int size){
  ...
  for (int i = 0; i < num-1; i++) {
	  MoveToEx(hDC, points[i].x, points[i].y, NULL); //сделать текущими координаты x1, y
	  LineTo(hDC, points[i+1].x, points[i+1].y);	
  }
  
	  MoveToEx(hDC, points[num-1].x, points[num-1].y, NULL); //сделать текущими координаты x1, y
	     ...
	     for (int i = 0; i < id; i++){
          if (id == 3 && i == 2) {
              ...
              if (points[i].x <= xCntr+(Radius/2)*sqrt(2)){
                  ...
                  points[i].y = yCntr - sqrt(pow(Radius,2) - pow((points[i].x >= xCntr)*(points[i].x - xCntr)+(points[i].x < xCntr)*(xCntr -points[i].x),2));
              	
							} else{
              	  points[i].y = points[i].y+Step;
                  points[i].x = xCntr + sqrt(pow(Radius,2) - pow((points[i].y >= yCntr)*(points[i].y - yCntr)+(points[i].y < yCntr)*(yCntr -points[i].y),2));
...
/*		if (points[2].x + 0.1 > pointscopy[2].x && points[2].x - 0.1 < pointscopy[2].x|| 
		    points[2].y + 0.1 > pointscopy[2].y && points[2].y - 0.1 < pointscopy[2].y )	{
			flag = 1;
		}			*/
					}
